...
public:
  ...
  Eigen::Matrix<float,2,1> currentError()       const { return lin_ang_error;}
...
private:
  ...
  Eigen::Matrix<float,2,1> lin_ang_error;
  ...
  float linear_thresh_mm_ = 10.0f;   // 0.15 mm
  ...
  // Eigen::Matrix<float,6,1> q_home_ = (Eigen::Matrix<float,6,1>() <<
  //                                     deg2rad(175.88f), deg2rad(87.8f), deg2rad(180.0f),
  //                                     deg2rad(180.0f),   deg2rad(228.7f), deg2rad(34.2f)).finished(); // rad
  // Eigen::Matrix<float,6,1> q_home_ = (Eigen::Matrix<float,6,1>() <<
  //                                     deg2rad(0.0f), deg2rad(0.0f), deg2rad(0.0f),
  //                                     deg2rad(0.0f), deg2rad(0.0f), deg2rad(0.0f)).finished(); // rad

  // DH Table（「小手臂」）
  float a_[6]     = {0, 245.0f, 0, 0, 0, 0};                   // (mm)
  float d_[6]     = {0, 0, 0, 302.0f, 0, 0};                   // (mm)
  float alpha_[6] = {-M_PI/2, 0, -M_PI/2, M_PI/2, -M_PI/2, 0}; // (rad)
